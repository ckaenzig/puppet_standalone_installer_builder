...
task :build_tarball => [:build_check, :reprepro, :spec_prep, :spec_standalone, :build_pdf_doc] do
  ...
  sh "tar cvzfh #{tarball} --owner=root --group=root #{readme} #{changelog} #{packages} #{examples} --exclude-from .gitignore --exclude .git --exclude #{apt_dir}/conf --exclude #{apt_dir}/lists --exclude #{apt_dir}/db -C spec/fixtures ENDUSER.pdf bin/ manifests/ --exclude manifests/site.pp modules/ --exclude modules/#{profile}/spec/fixtures/modules --exclude modules/#{profile}/packages --transform 's,^,#{base_path}/,'"
